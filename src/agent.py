...
## 🚫 全局输出格式规则（所有 Sub-agent 必须遵守）

**所有 Sub-agent 的输出中，绝对禁止使用任何 HTML 标签。**

- ❌ 禁止: `<br>`, `<b>`, `<strong>`, `<div>`, `<span>` 等
- ✅ 换行请使用普通换行符（按 Enter 空行）
- ✅ 强调请使用 Markdown: `**粗体**`、`*斜体*`

违反此规则的输出将被自动判定为格式错误并要求重写。
